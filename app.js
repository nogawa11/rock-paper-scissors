...
const resultsText = document.querySelector(".results-details-text");
...
const housePlay = () => {
  ...
  const choices = [scissorsDiv, paperDiv, rockDiv];
  const houseSelect = choices[randomNum];
  return houseSelect;
}

const getResults = (player, houseSelect) => {
  let win = false;
  if (player === paper && houseSelect === paperDiv) {
    win = undefined;
  } else if (player === rock && houseSelect === rockDiv) {
    win = undefined;
  } else if (player === scissors && houseSelect === scissorsDiv) {
    win = undefined;
  } else if (player === paper && houseSelect === rockDiv) {
    win = true;
  } else if (player === rock && houseSelect === scissorsDiv) {
    win = true;
  } else if (player === scissors && houseSelect === paperDiv) {
    win = true;
  } else {
    win = false;
  }
...
const getResultsMessage = (results) => {
  let message = '';
  if (results === true) {
    message = "You win";
...
const playGame = (player) => {
  ...
  playerChoice.insertAdjacentElement("beforeend", player);
  const results = getResults(player, houseSelect);
  const message = getResultsMessage(results);
  setTimeout(function () {
    loading.classList.add("hide");
    houseChoice.insertAdjacentHTML("beforeend", houseSelect);
    setTimeout(function () {
      resultsDetails.classList.remove("hide");
...
playAgainBtn.addEventListener("click", (event) => {
  ...
  gameDiv.insertAdjacentElement("beforeend", rock);
  gameDiv.insertAdjacentElement("beforeend", paper);
  gameDiv.insertAdjacentElement("beforeend", scissors);
  resultsDetails.classList.remove("add");
  loading.classList.remove("hide");
  resultsDetails.classList.add("hide");
